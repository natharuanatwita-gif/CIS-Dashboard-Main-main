"""
calculate_modules/fair_value.py — v3 (Updated with Sector Baseline & Full Audit Fixes)
====================================================================================
...
คืนค่าเป็น dict ที่ครอบคลุมคีย์ทั้งหมด ทั้งรูปแบบเดิมและใหม่เพื่อความเข้ากันได้ 100%:
    valuation_score, fair_value, dcf_fair_value, pe_fair_value,
    margin_of_safety, pe_ratio, pb_ratio, market_cap_mb, eps,
    wacc, wacc_used, terminal_growth, terminal_growth_used,
    target_pe, target_pb, fcf_growth_assumed, fcf_base_used,
...
=== CHANGELOG & AUDIT FIXES (ISSUE 01 - 06) ===
[ISSUE 01] ปลดล็อค Clip 65%-185% ออกจากการแสดงผล:
           dcf_fair_value, pe_fair_value และ blended fair_value ส่งออกเป็นค่าดิบที่คำนวณได้จริง
           บันทึก raw_dcf_fair_value, raw_pe_fair_value และ flag is_clipped / dcf_is_clipped / pe_is_clipped
           นำ np.clip ไปใช้เฉพาะขั้นตอนคำนวณคะแนน (calc_sub_score) เท่านั้น
[ISSUE 02] ยกเลิกตรรกะ fallback 85% (PE) และ 90% (DCF) สำหรับบริษัทขาดทุน:
           หาก EPS <= 0 ให้ PE Fair Value เป็น None
           หาก FCF <= 0 ให้ DCF Fair Value เป็น None
           โมเดลที่คำนวณไม่ได้จะได้คะแนนย่อย 0 คะแนน (Penalty)
           หากประเมินไม่ได้ทั้งคู่ blended fair_value เป็น None, valuation_status = 'NOT_RATED'
[ISSUE 03] แยกคะแนนย่อย pe_score และ dcf_score จากกันอย่างแท้จริง ผ่าน calc_sub_score()
           ไม่ใช้ valuation_score ตัวเดียวซ้ำสองมิติ
[ISSUE 04] ปรับระบบ Confidence Level เป็น Risk Penalty System:
           - Low: ขาดทุน (EPS <= 0 หรือ FCF <= 0) หรือประเมินไม่ได้
           - Medium: ราคาดิบพุ่งชนขอบ Clip (is_clipped == True) หรือ MOS ทั่วไป
           - High: กำไรเป็นบวก + ค่าดิบไม่ชนกรอบ + Margin of Safety > 15%
[ISSUE 05] Guard Clause ตรวจจับ Empty DataFrame และ current_price <= 0 ป้องกันระบบล่ม
[ISSUE 06] กำหนดค่า WACC, Target P/E และ Terminal Growth ตาม 3 กลุ่มอุตสาหกรรม (Sector Baseline Benchmark):
           - กลุ่มที่ 1: Tech & Telecomm (ADVANC, TRUE, THCOM) -> Target P/E 22.0x, WACC 7.8%, g 2.0%
           - กลุ่มที่ 2: Electronic Components (DELTA, HANA, KCE, CCET) -> Target P/E 18.0x, WACC 8.8%, g 1.5%
           - กลุ่มที่ 3: Commerce & Tech (JMART) -> Target P/E 22.0x, WACC 9.2%, g 2.5%
"""
...
# 2. การกำหนดสมมติฐานตามกลุ่มอุตสาหกรรม (Sector Baseline Benchmark)
STOCK_VALUATION_PARAMS = {
    # 📊 กลุ่มที่ 1: Technology & Telecommunication
    # ผู้ขายเท่าน้อยราย (Oligopoly), กระแสเงินสดสม่ำเสมอ, ขยายโครงข่ายดิจิทัลต่อเนื่อง
    ...
    # รายได้หลักจากการส่งออก ผันผวนตาม Global Tech Cycle และค่าเงิน WACC สูงขึ้นตาม Beta
    ...
    # Retail & Holding ผสมผสานการเงิน/เทคโนโลยี การเติบโตสูงแต่ผันผวนตามกำลังซื้อและการจัดเก็บหนี้
...
_EMPTY_KEYS = [
    ...
    'wacc', 'wacc_used', 'terminal_growth', 'terminal_growth_used', 'fcf_growth_assumed', 'fcf_base_used',
    'target_pe', 'target_pb',
    'raw_dcf_fair_value', 'raw_pe_fair_value', 'dcf_fair_value_raw', 'pe_fair_value_raw',
    'dcf_is_clipped', 'pe_is_clipped', 'is_clipped',
    'pe_score', 'dcf_score', 'intrinsic_score', 'relative_score', 'safety_score',
...
def calculate_valuation_module(df_fin_ticker, current_price, ticker, beta=None, **kwargs):
    ...
    รองรับทั้งการเรียกผ่าน calculate_scores.py (ส่ง beta=...) และหน้า UI pages_content/fair_value.py
    """
    ...
    # ดึงพารามิเตอร์ตามกลุ่มหุ้น
    param = STOCK_VALUATION_PARAMS.get(ticker)
